...
class ProteinLangaugeModelDataset(Dataset):
    def __init__(self, data, tokenizer, seq_len=128, masking_rate=0.15):
        super(ProteinLangaugeModelDataset, self).__init__()
...
def generate_epoch_dataloader(data, seq_len, tokenizer, masking_rate, batch_size, collate_fn, shuffle=True, num_workers=6):
    dataset    = ProteinLangaugeModelDataset(data=data, seq_len=seq_len, tokenizer=tokenizer, masking_rate=masking_rate)
