...
class MoleculeLangaugeModelDataset(Dataset):
    def __init__(self, data, tokenizer, seq_len, masking_rate):
        super(MoleculeLangaugeModelDataset, self).__init__()

        self.data = data        
        
        self.stoi = tokenizer[0]
        self.itos = tokenizer[1]
    ...
    def __getitem__(self, idx):
        target = torch.tensor([self.stoi[s] for s in self.data[idx]])
        ...
        return train, target, masking_label
